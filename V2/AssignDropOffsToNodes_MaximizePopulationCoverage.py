...
def check_for_uncovered_units(G):
    return bool(sum(float(edge[2]['AdjUnitsRes']) for edge in list(G.edges.data()) if 'AdjUnitsRes' in edge[2]))
...
def calculate_node_weights(G):
    for node in list(G.nodes):
        ...
        edge_weights_list = [int(float(edge[2]['AdjUnitsRes'])) for edge in list(ego_graph.edges.data()) if
                             'UnitsRes' in edge[2]]
...
nyc_graph = ox.load_graphml('nyc_graph_3_sumEdgeUnitsPerNode.graphml')
...
while uncovered_units:
    ...
    uncovered_units = check_for_uncovered_units(nyc_graph)
    try:
        ox.plot_graph(nyc_graph, node_size=0, show=False, save=True, filepath=rf'animationFrames/v2run1/manhattan/mn_frame_{str(loop_count)}.png', close=True)
        ox.plot_graph(ego_graph, node_size=0, show=False, save=True, filepath=rf'animationFrames/v2run1/localstreets/local_frame_{str(loop_count)}.png', close=True)
    except:
        pass
