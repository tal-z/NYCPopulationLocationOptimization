...
print(building_footprints_gdf['ZipCode'])
...
edges_residents_df = building_footprints_gdf[['allocated_edge','UnitsRes']].groupby(['allocated_edge']).sum()
edges_residents_df.sort_values(by='UnitsRes', inplace=True)
...
nyc_edges_gdf = nyc_edges_gdf[['u', 'v', 'key', 'osmid', 'name', 'highway',
                               'oneway', 'length', 'geometry', 'allocated_edge', 'UnitsRes']]
...
ox.save_graphml(nyc_graph, 'graphFiles/graphml/mn_step1_allocateUnitsToEdges_20210418.graphml')
